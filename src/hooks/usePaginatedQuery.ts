...
export function usePaginatedQuery(
  { defaultSize = 12, defaultSort }: { defaultSize?: number; defaultSort?: string } = {},
) {
  ...
  const setParams = React.useCallback(
    (patch: Record<string, any>, { replace = false }: { replace?: boolean } = {}) => {
      setSearchParams(
        (prev) => {
  ...
  const setPage = React.useCallback((p: number) => setParams({ page: p }), [setParams]);
