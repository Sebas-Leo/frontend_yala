...
http.interceptors.request.use((config) => {
  ...
  // `auth` is a custom flag we add on top of the Axios config.
...
http.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (response.status === 401 && config && (config as any).auth !== false && !(config as any)._retry) {
...
export const api = {
  get: <T = any>(path: string, opts?: any): Promise<T> => http.get(path, opts).then((r) => r.data),
  post: <T = any>(path: string, body?: any, opts?: any): Promise<T> => http.post(path, body, opts).then((r) => r.data),
  put: <T = any>(path: string, body?: any, opts?: any): Promise<T> => http.put(path, body, opts).then((r) => r.data),
  patch: <T = any>(path: string, body?: any, opts?: any): Promise<T> => http.patch(path, body, opts).then((r) => r.data),
  del: <T = any>(path: string, opts?: any): Promise<T> => http.delete(path, opts).then((r) => r.data),
};
